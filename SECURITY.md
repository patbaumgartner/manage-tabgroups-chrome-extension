...
| Losing tabs the extension cannot represent                         | Only tabs that were successfully written into a session are closed, and only while they still hold the stored URL, have no navigation in flight to somewhere else, and are still in a group |
