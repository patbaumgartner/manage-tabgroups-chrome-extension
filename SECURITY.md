...
| Data exfiltration                                                  | No host permissions, `default-src 'none'` with `connect-src 'none'`, and no `fetch`, `XMLHttpRequest`, `WebSocket` or `sendBeacon` in the bundle. Remote images, stylesheets and CSS `url()` references are rejected by the validator too |
