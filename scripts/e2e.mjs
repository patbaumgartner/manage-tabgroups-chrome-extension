...
console.info(`Launching a browser${HEADLESS ? ' (headless)' : ''}`);

const session = await launchWithExtension({ extensionDir: EXTENSION_DIR, headless: HEADLESS });
...
if (failure) {
  ...
  const tail = session.browserLog().join('').trim().split('\n').slice(-15).join('\n');
  if (tail.length > 0) {
    console.error(`\nBrowser output (tail):\n${tail}`);
  }
  process.exit(1);
